...
import { Volunteer, Role, TrainingModule, Department, Journey } from '../types';
              ...
              <tr key={v.id} className="hover:bg-blue-50/30 transition-colors group">
                ...
                <td className="px-10 py-6">
                  <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button 
                      onClick={() => onViewAs(v.id)} 
                      className="px-4 py-2 bg-blue-50 text-blue-600 rounded-xl font-black uppercase text-[10px] hover:bg-blue-600 hover:text-white transition-all shadow-sm"
                    >
                      View As
                    </button>
                    <button 
                      onClick={() => setEditingId(v.id)} 
                      className="px-4 py-2 bg-gray-50 text-gray-600 rounded-xl font-black uppercase text-[10px] hover:bg-gray-200 transition-all shadow-sm"
                    >
                      Edit Roles
                    </button>
                    <button 
                      onClick={() => setResettingId(v.id)} 
                      className="px-4 py-2 bg-red-50 text-red-600 rounded-xl font-black uppercase text-[10px] hover:bg-red-600 hover:text-white transition-all shadow-sm"
                    >
                      Reset
                    </button>
                  </div>
              ...
              {roles.map(r => {
                ...
                return (
                  <button 
                    ...
                    className={`w-full text-left p-4 rounded-2xl border-2 transition-all flex items-center justify-between group ${isSelected ? 'bg-blue-600 border-blue-600 text-white shadow-lg' : 'bg-gray-50 border-transparent hover:border-blue-100'}`}
                  >
                    <div>
                      <p className={`font-bold text-sm ${isSelected ? 'text-white' : 'text-gray-900'}`}>{r.name}</p>
                      <p className={`text-[10px] font-black uppercase tracking-widest mt-0.5 ${isSelected ? 'text-blue-100' : 'text-gray-400'}`}>
          ...
          <div className="bg-white p-10 rounded-[2.5rem] w-full max-w-xl shadow-2xl relative flex flex-col max-h-[90vh]">
            <div className="mb-8">
              ...
              <p className="text-sm text-gray-500">
                Manage completed chapters for <span className="font-bold text-gray-900">{resettingVolunteer.fullName}</span>.
              </p>
            </div>
                    ...
                    <div key={m.id} className="p-5 bg-white border border-gray-100 rounded-3xl shadow-sm flex items-center justify-between group">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-1">
                          <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded ${m.isCompulsory ? 'bg-red-50 text-red-500' : 'bg-blue-50 text-blue-500'}`}>
                            {m.isCompulsory ? 'Foundational' : 'Specialized'}
                          </span>
                          <span className="text-xs font-bold text-gray-900">{m.title}</span>
                        </div>
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                          {completedInModule} / {totalChapters} Chapters Completed
                        </p>
                      </div>
                      <button 
                        disabled={completedInModule === 0}
                        onClick={() => onResetProgress(resettingVolunteer.id, m.id)}
                        className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${completedInModule > 0 ? 'bg-red-50 text-red-500 hover:bg-red-500 hover:text-white' : 'bg-gray-50 text-gray-300 cursor-not-allowed'}`}
                      >
                        Reset Module
                      </button>
                    </div>
            ...
            <div className="pt-8 border-t border-gray-100 grid grid-cols-2 gap-4">
              <button 
                onClick={() => {
                  if (confirm(`Completely wipe ALL progress for ${resettingVolunteer.fullName}? This cannot be undone.`)) {
                    onResetProgress(resettingVolunteer.id);
                  }
                }} 
                className="bg-red-600 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-red-700 transition-all shadow-lg shadow-red-100"
              >
                Reset All Progress
              </button>
              <button 
